...
export function Header({ search, onSearchChange, onCarModeEnter }: HeaderProps) {
  ...
  return (
    <header
      className={`sticky top-0 z-40 backdrop-blur border-b px-4 py-3 ${isDarkMode ? 'bg-[#0f0f0f]/95 border-white/5' : 'bg-white/95 border-gray-200 shadow-sm'}`}
    >
      <div className="max-w-7xl mx-auto flex items-center gap-3">
        ...
        <div className="flex-1 relative">
          ...
          {search && (
            <button
              ...
              className={`absolute left-3 top-1/2 -translate-y-1/2 transition-colors ${isDarkMode ? 'text-white/40 hover:text-white/80' : 'text-gray-400 hover:text-gray-700'}`}
        ...
        <button
          ...
          className={`shrink-0 p-2 rounded-lg transition-colors sm:hidden ${isDarkMode ? 'text-white/60 hover:text-white hover:bg-white/8' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'}`}
          aria-label="מצב רכב"
          title="מצב רכב"
        >
          <CarIcon />
        ...
        <button
          ...
          className={`shrink-0 p-2 rounded-lg transition-colors ${isDarkMode ? 'text-white/60 hover:text-white hover:bg-white/8' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'}`}
